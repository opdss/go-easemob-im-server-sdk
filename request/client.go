...
type Client interface {
	...
	Get(ctx context.Context, apiPath string, resp any) error
	Post(ctx context.Context, apiPath string, req any, resp any) error
	Put(ctx context.Context, apiPath string, req any, resp any) error
	Delete(ctx context.Context, apiPath string, req any, resp any) error
}
...
func (c *client) request(req *http.Request, resp any) error {
	...
	if c.config.Debug {
		log.Println("easemob[client.request] response: ", buf.String())
	}
	if err != nil {
		return &RequestErrorResp{Code: httpResp.StatusCode, Err: err, Body: buf.String()}
	}
	if httpResp.StatusCode != 200 {
		errResp := ErrorResp{}
		err = json.NewDecoder(buf).Decode(&errResp)
		if err != nil {
			return &RequestErrorResp{Code: httpResp.StatusCode, Err: err, Body: buf.String()}
		}
	...
	return json.NewDecoder(buf).Decode(&resp)
}
...
func (c *client) getToken(ctx context.Context) (*TokenResp, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, c.getApiUrl("/token"), buf)
	if err != nil {
		return nil, err
	}
	resp := TokenResp{}
